...
更新：根据[ECMAScript规范](https://tc39.es/ecma262/#sec-array.prototype.shift),这个时间复杂度就是O(n)的，所以尽量不要使用数组的shift和push来模拟队列
#### 栈
...
export function reverseKGroup(head: ListNode, k: number): ListNode {
    function reverse(head: ListNode): ListNode {
        if (!head) return null;
        //不足k个直接返回
        for (let cnt = 0, p = head; cnt < k - 1 && p; p = p.next, ++cnt) {
            if (cnt !== k - 1 && p.next === null) return head;   
        }
        let curr = head.next,
            prev = head,
            i = 0;
...
export function reverseKGroup(head: ListNode, k: number): ListNode {
    if (!head) return null;
    let dummy = new ListNode(),
        preTail = dummy,//上一个k部分的尾巴
        currHead = head;//本次k部分的开头
    while (currHead) {//currHead必须不是null
        let doReverse = (k !== 1);
        for (let p = currHead, i = 0; p && i < k - 1; ++i, p = p.next) {
            if (i !== k - 1 && !p.next) {
                doReverse = false;
                break;
            }
        ...
        if (doReverse) {
            let curr = currHead.next,//这里只能在k>=2时执行
                prev = currHead;
            for (let i = 0; i < k - 1 && curr; ++i) {
                let next = curr.next;
                curr.next = prev;
                prev = curr;
                curr = next;
...
export function maxWater(arr: number[]): number {
    const n = arr.length;
    let l: number[] = Array(n).fill(0),
        r: number[] = Array(n).fill(0);
    for (let i = 0; i < n; ++i) {
        l[i] = i === 0 ? arr[i] : Math.max(arr[i], l[i - 1]);
    }
    for (let i = n - 1; i >= 0; --i) {
        r[i] = i === n - 1 ? arr[i] : Math.max(arr[i], r[i + 1]);
    }
    let ans = 0;
    for (let i = 0; i < n; ++i) {
...
export function IsBalanced_Solution(pRoot: TreeNode): boolean {
    function isBalanced(root: TreeNode): [boolean, number]{
        if (!root) return [true, 0];
        let [lBalanced, lHeight] = isBalanced(root.left),
            [rBalanced, rHeight] = isBalanced(root.right);
        if (lBalanced && rBalanced && Math.abs(lHeight - rHeight) <= 1)
