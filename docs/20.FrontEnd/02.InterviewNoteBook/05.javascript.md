---
title: JavaScript
date: 2022-03-19 23:54:08
permalink: /pages/502454/
categories:
  - FrontEnd
  - InterviewNoteBook
tags:
...
转载自[前端面试笔记](https://github.com/CavsZhouyou/Front-End-Interview-Notebook)
...
//  缺点：<del>每个元素被派到新数组的位置不是随机的，原因是 sort() 方法是依次比较的。</del>
//2022.3.22注：这是错误的实现方式，不要使用这种方式!!!!!!!!
...
延申：Array.sort()是使用什么排序算法实现的

回答：
在Mozilla中使用merge sort，在v8中使用quick sort和merge sort
...
[MDN关于this的文档](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/this)
...
![js运行图示](https://mdn.mozillademos.org/files/17124/The_Javascript_Runtime_Environment_Example.svg)
...
参考《JS忍者秘籍》
![事件循环示意图](https://img.jyehn.xyz/c103e8c57bcb1fd7a3610f6df3700d6c68549c42d4bbd84f84241bd7be7fe911.png)
...
微任务包括了 promise.then()/catch()、Promise为基础开发的其他技术比如fetch API、V8的垃圾回收、node 中的 process.nextTick 、对 Dom 变化监听的 MutationObserver。
...
2022.4.1 更新：virtual DOM的作用不止这一些
尤雨溪在vue3 Mastery 课程中说
1. virtual DOM将渲染的逻辑从实际的DOM中解耦出来，让其在其他非浏览器环境中更容易使用，比如说渲染成一个字符串（SSR），渲染
到canvas/WebGL，或者是移动端原生渲染
2. 提供了一个能够程序化地构建、检查、复制、创建指定的结构， 可以利用JavaScript语言的能力来做到这些。因为在一些UI编程中，使用模板语言总是有局限性的，需要使用一种灵活的编程语言才能表达想要创建的结构。
...
## 176. 页面刷新后vuex实例里面store的数据会丢失吗，应该怎么办
回答：刷新网页后vuex实例里面的数据会丢失， 因为store里的数据是保存在运行内存当中，当刷新页面的时候，页面会重新加载Vue实例，store里面的数据会被重新赋值初始化。

解决方法：将state的数据保存在localStorage,sessionStorage或者cookie中
最好是保存在sessionStorage中（原因见三者的区别）
...
## 179. Vue的keep-alive的实现

## 180. Vue2/Vue3的依赖收集差别



## 181. Vite的原理，为什么在运行时用esbuild, 打包用rollup

## 182. 一个Vue文件时怎么被打包成js的，并最终被浏览器解析
...
## 186. express和koa的区别
1. 中间件模型
   Express的中间件模型为线性，Koa的中间件模型为U型（洋葱型）
2. 异步方式
   Express通过回调实现异步函数，在多个回调、多个中间件中写起来容易逻辑混乱。Koa通过generator和async/await使用同步的写法来处理异步
3. 捕获错误
   Express沿用Node.js的Error-First的模式（第一个参数是error对象）来捕获错误
   Koa使用try/catch的方法来捕获错误
4. 响应机制
   Express立刻响应(res.json/res.send)，上层不能再定义其他处理
   Koa中间件执行完之后才能响应（ctx.body = xxx），每一步都可以响应自己的处理
...
Vuex是专门用来为Vue服务，用于管理页面的数据状态，提供统一数据操作的状态管理系统。

![Vuex工作原理图](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9ffcbd2177394e73a198e9ef101a55f9~tplv-k3u1fbpfcp-zoom-in-crop-mark:1304:0:0:0.awebp)
...
相关文章:[Vuex原理解析](https://www.jianshu.com/p/87db06d5b84e) [理解Vuex原理](https://juejin.cn/post/6844903949938475022)
...
```js
let input = [
    {id:0,name:'xiaoming',pid:null},
    {id:1,name:'xiaohong',pid:0},
    {id:2,name:'xiaohua',pid:0},
    {id:3,name:'xiaolan',pid:2},
    {id:4,name:'xiaoke' ,pid:3},
    {id:5,name:'laoba', pid: 4},
    {id:6,name:'keke', pid:5},
    {id:7,name:'hehe', pid:6},
    {id:8,name:'titian', pid:2},
    {id:9,name:'99', pid:4},
]
...
function convert(input) {
    ...
    input.forEach((item) => {
        if (item.pid === null) {
            ...
            if (!children.has(item.pid)) {
                children.set(item.pid, [])             
            }
            let arr = children.get(item.pid);
            arr.push(item.id);
        }
    })
    ...
    function dfs(id) {
        let p = {};
        p.id = id;
        p.name = input[id].name;
        p.children = [];
        if (children.has(p.id)) {
            let arr = children.get(p.id);
            for (let cid of arr) {
                p.children.push(dfs(cid));
            }
        }
        return p;
    }
...
![图片](https://img.jyehn.xyz/bd159df65cf217a6e22b9eaa166bacab.png)
...
Promise.race = function(_promises) {
    return new Promise((resolve, reject) => {
        for (let i = 0; i < _promises.length; ++i) {
            Promise.resolve(_promises[i]).then(o => {
                resolve(o);
            }).catch(e => {
                reject(e);
            })
...
function flatten(arr) {
    return arr.reduce((p, value) => {
        if (Array.isArray(value)) {
            p = p.concat(flatten(value));
        }else {
            p.push(value);
        }
        return p;
...
Array.prototype.myReduce = function(fn, ...initial) {
    let next = initial.length ? initial[0] : this[0];
    for (let i = initial.length ? 0 : 1; i < this.length; ++i) {
      next = fn(next, this[i]);
    }
    return next;
}
...
// let arr = [1,2,3,4,5,6];
// arr.myReduce((pre, curr)=> {
//     console.log(pre);
//     return pre + curr;
// }, 0)
```

## 实现String.prototype.trim
题目：如何去除字符串首位空白字符
...
```
## 实现一个可以拷贝内置对象、循环应用的深拷贝
对Set，Map,Date，RegExp等等内置对象特殊处理
使用一个WeakMap将已经复制过的对象保存
```js
function cloneDeep(src, mem) {
    const toString = Object.prototype.toString;
    const isPrimitive = (value) => {
        return /Number|Boolean|String|Null|Undefined|Symbol|Function/.test(toString.call(value));
    }
    function isType(typeStr, value) {
        return toString.call(value) === `[object ${typeStr}]`;
    }
    
    let result = null;

    mem || (mem = new WeakMap());//mem是已经复制过的对象
    if (isPrimitive(src)) {
        console.log('current copy is primitive', src);
        result = src;
    }else if (Array.isArray(src)) {
        result = src.map(value => cloneDeep(value, mem));
    }else if (isType('Date', src)) {
        result = new Date(src);
    }else if (isType('RegExp', src)) {
        result = new RegExp(src);
    }else if (isType('Set', src)) {
        result = new Set();
        for (const value of src) {
            result.add(cloneDeep(value, mem));
        }
    }else if (isType('Map', src)) {
        result = new Map();
        for (const [key, value] of src.entries()) {
            result.set(key, cloneDeep(value, mem));
        }
    }
    else {
        if (mem.has(src)) {//循环引用
            ...
            Object.keys(src).forEach(key => {
                result[key] = cloneDeep(src[key], mem);
            })
        }
    }
    return result;
...
console.log(cloneDeep(xiaoming, null));
console.log(cloneDeep(obj, null));
```
## 实现lodash.get()

先使用正则表达式将[]转化成通过.调用的属性，然后沿着对象属性指向找到值。
```js
...
function maxBy(data, fn) {
    ...
    data.forEach((item, index) => {
        ...
        }else if (maxVal === fn(item)) {
            maxIndices.push(index);
        }
    })
    return maxIndices.map((index) => data[index]);
}
...
创建一个由集合中的对象迭代生成的对象，这个对象的key是指定的值，value是数组中原来的对象
```js
// Output: {
//   "1": {
//     "id": 1,
...
//=> [2]
let res1 = intersection([2, 1], [2, 3]);
console.log(res1);
//=> [1, 2]
let res2 = intersection([1, 2, 2], [1, 2, 2])
console.log(res2);
//=> [1, 2]
let res3 = intersection([1, 2, 2], [1, 2, 2], [1, 2])
console.log(res3)
```
## 实现promise.map，控制promise并发数
思路：使用一个标记currentIndex记录下一个可进行的任务，使用一个标记completeCount记录完成的任务。

当一个任务完成之后，开始下一个任务(currentIndex++)，当completeCount等于任务数量，即所有任务已完成
```js
Promise.map = function (input, mapper, concurrency=Infinity) {
...
答:
1. 串行的写法，每次计算完2个的和，产生一个promise再和下一个数算和
2. 并行的写法，每次2个一组执行，产生一个数量为原来一半的promise数组，然后对这个promise数组再2个一组执行
直到数组里只有1个元素
```js
//1.串行的写法
function sum(arr) {
...
字符集是字符和编码一一对应的一个映射表，常见的字符集有ASCII、Unicode等。

Unicode中重要的一个概念就是码点，码点是字符集中唯一表示某个字符的标识。编码过程分为两步，第一步是将
字符转化为码点，第二部是将码点以特定的方式存储到计算机中。也就是说，码点不是最终存储的值 。、

Unicode字符集衍生出来的编码方案有三种，分别是UTF-32，UTF-16和UTF-8。
UTF-32: 在前面填充0满4字节即可
UTF-16 UTF-8都是变长编码
