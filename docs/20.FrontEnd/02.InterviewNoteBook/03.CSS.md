---
title: CSS
date: 2022-03-20 09:57:37
permalink: /pages/83caa6/
categories:
  - FrontEnd
  - InterviewNoteBook
tags:
...
一级position设置为absolute或者relative的父元素的padding box的左上角为原点的。文档会被移出正常文档流。

fixed（老IE不支持）

生成绝对定位的元素，相对于浏览器窗口进行定位。文档会被移出正常文档流。当元素祖先的 transform, perspective 或 filter 属性非 none 时，容器由视口改为该祖先。打印时，元素会出现在的每页的固定位置。

relative

生成相对定位的元素，相对于其元素本身所在正常位置进行定位。position:relative 对 table-*-group, table-row, table-column, table-cell, table-caption 元素无效。
...
相对正常文档流进行定位，然后相对于它的最近滚动祖先和最近块级祖先，包括table-related元素，基于top, right, bottom, 和 left的值进行偏移。偏移值不会影响任何其他元素的位置。会创建一个新的层叠上下文，sticky元素总是固定在离它最近的一个拥有滚动机制的祖先上（当该祖先的overflow 是 hidden, scroll, auto, 或 overlay时）。可简单理解为fixed和relative的结合
...
（3）在很多CSS属性上有自己的一套表现规则。比较具有代表性的就是vertical-align属性，对于替换元素和非替换元素，
vertical-align属性值的解释是不一样的。比方说vertical-align的默认值的baseline，很简单的属性值，基线之意，
...
（2）HTML尺寸只能通过HTML原生属性改变，这些HTML原生属性包括`<img>`的width和height属性、`<input>`的size
属性、`<textarea>`的cols和rows属性等。
...
![层叠顺序](http://img.jyehn.xyz/dad810372dd9075348d9bb7189be0e74.png)
...
## 106. 如何开启GPU硬件加速
如何开启GPU加速：当浏览器检测到页面中某个DOM元素应用了某些CSS规则时，就会开启
...
```
可以使用`transform: translateZ(0)`来开启硬件加速
```
.cube {
...
```
在在 Chrome and Safari中，当我们使用CSS transforms 或者 animations时可能会有页面闪烁的效果，下面的代码可以修复此情况：
```
.cube {
...
## 108. js html css文件缓存在磁盘上还是内存里

css放在磁盘中，js、字体、图片放在内存中

## 实现div垂直水平居中，并且div高度永远是宽度的一半
特别注意这里实现高度和宽度比例的技巧：设置一个这个父元素的div为相对定位，高度为0，下padding为50%，然后设置这个div
为绝对定位，宽度和高度都是100%
这里利用了绝对定位的高度和宽度百分比是相对与定位上下文的padding box
...
```
2. 父级text-align: center，子级inline-block + text-align: left
```html
  <div class="father">
    <div class="text">测试单行文字居中，多行文本左对齐。测试单行文字居中，多行文本左对齐。</div>
  </div>
